...
# 你应该知道的setTimeout秘密
http://ghmagical.com/article/page/id/H61NOVU0RZ9Y
...
---
# setTimeout的经典面试题答案汇总
题目为: 简单改下以下代码，输出1，2，3，4，5
```js
for(var i = 0; i < 5; i++){
   setTimeout(function(){
      console.log(i);
...
//输出1，2，3，4，5
```
解题思路为：一是使用自执行函数提供闭包条件，二是传入i值并保存在闭包中。

2.setTimeout第三个参数 来自（原文: 你应该知道的setTimeout秘密 © ghmagical.com）
```js
for(var i = 0; i < 5; i++){
   setTimeout(function(i){
      console.log(i+1);
...
//1,2,3,4,5
```
解题思路：setTimeout第三、四。。。。个参数为第一个参数（回调函数）的参数。

3.利用bind方法 （波老师图例详解那道setTimeout与循环闭包的经典面试题的评论区的风萧萧梦潇和微醺岁月）
```js
//写法1
for(var i = 0; i < 5; i++){
   setTimeout(function(i){
...
//写法2
for(var i = 0; i < 5; i++){
   setTimeout(console.log.bind(console,i+1),1000*i)
}
```

4.利用ES6的let (谈谈 setTimeout 这道经典题目@dailybird)
```
for(let i = 0; i < 5; i++){
    setTimeout(function(){
	console.log(i+1)
